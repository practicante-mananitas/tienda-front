...
export class CheckoutComponent implements OnInit {
  ...
  calcularSubtotal() {
    this.totalProductos = this.cartItems.reduce((acc, item) => {
      ...
      return acc + (Number(precio) * item.quantity);
    }, 0);
  }
  ...
  confirmarCompra() {
    ...
    const payload = {
      items: this.cartItems.map(item => {
        ...
        const imageBaseUrl = `${environment.apiUrl}/storage/`; // <-- aquí usa environment.apiUrl
    ...
    this.http.post<any>(`${environment.apiUrl}/pago/preferencia`, payload, {
