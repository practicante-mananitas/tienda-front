...
@Component({
  selector: 'app-admin-pedidos-entregados',
  standalone: true,
  imports: [
    CommonModule,
    DatePipe,
  ...
  templateUrl: './admin-pedidos-entregados.component.html',
  styleUrls: ['./admin-pedidos-entregados.component.scss']
})
export class AdminPedidosEntregadosComponent implements OnInit {
  pedidos: any[] = [];
  ...
  cargarPedidos() {
    ...
    this.http.get<any[]>(this.apiBase).subscribe({
      next: (data) => {
        this.pedidos = data.filter(pedido => pedido.shipment_status === 'delivered');
